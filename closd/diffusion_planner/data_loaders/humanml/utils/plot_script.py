...
def plot_3d_motion(
    ...
    def update(index):
        ...
        if cond is not None and "heading_print" in cond.keys():
            hp = cond["heading_print"]
            # Handle scalar, 1D, or 2D heading_print
            if np.ndim(hp) == 0:
                _title += " [{}]".format(int(hp))
            elif np.ndim(hp) == 1:
                _title += " [{}]".format(int(hp[min(index, len(hp) - 1)]))
            else:
                _title += " [{}]".format(int(hp[min(index, hp.shape[0] - 1)]))
    ...
    # Set explicit duration based on number of frames and fps
    duration = n_frames / fps
    ani = VideoClip(update, duration=duration)
