...
from closd.diffusion_planner.data_loaders.humanml.utils.plot_script import (
    plot_3d_motion,
)
import shutil
...
def main(args=None):
    ...
    fps = 12.5 if args.dataset == "kit" else (60 if args.dataset == "aistpp" else 20)
    # Determine n_frames: prefer args.n_frames, then motion_length, then max_frames
    if hasattr(args, 'n_frames') and args.n_frames is not None and args.n_frames > 0:
        n_frames = args.n_frames
        if not args.autoregressive:
             n_frames = min(max_frames, n_frames)
    elif args.motion_length is not None:
    ...
    if getattr(args, "audio_concat_mode", "none") == "concat":
        ...
        if "prefix" in y and audio_prefix is not None:
            ...
            audio_feat_prefix = audio_pref_trim.permute(0, 2, 1).unsqueeze(
                2
            )  # (B, F_audio, 1, T_prefix)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        if (
            "text" in model_kwargs["y"].keys()
            and getattr(model, "text_encoder_type", "clip") != "none"
        ):
        ...
        print(
            "model_kwargs['y']['audio_embed_prefix'].shape = ",
            model_kwargs["y"]["audio_embed_prefix"].shape,
        )
        print(
            "model_kwargs['y']['audio_embed_pred'].shape = ",
            model_kwargs["y"]["audio_embed_pred"].shape,
        )
        sample = sample_fn(
            model,
        ...
        heading_all = heading_all.cpu().numpy().round()
        # Keep as 2D array (batch, seq) to avoid scalar indexing issues in plot_script
        if heading_all.ndim > 2:
            heading_all = heading_all.reshape(heading_all.shape[0], -1)
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            # Check for audio to update caption
            audio_path_name = ""
            if "audio" in model_kwargs["y"] and model_kwargs["y"]["audio"] is not None:
                audio_meta = model_kwargs["y"]["audio"][sample_i]
                if audio_meta is not None:
                    audio_path = audio_meta.get("path")
                    if audio_path:
                        audio_path_name = os.path.basename(audio_path)
            ...
            # Add audio to the video clip if available
            if "audio" in model_kwargs["y"] and model_kwargs["y"]["audio"] is not None:
                audio_meta = model_kwargs["y"]["audio"][sample_i]
                if audio_meta is not None:
                    try:
                        # Try to load full audio from the original wav file
                        audio_path = audio_meta.get("path")
                        start_time = audio_meta.get("start_time", 0.0)
                        
                        # Calculate expected video duration from motion frames
                        video_duration = motion.shape[0] / fps
                        
                        if audio_path and os.path.exists(audio_path):
                            # Load full audio from wav file
                            from scipy.io import wavfile
                            sr, full_audio = wavfile.read(audio_path)
                            
                            # Convert to float32 and normalize
                            if full_audio.dtype == np.uint8:
                                # uint8 is 0-255, center at 128
                                full_audio = (full_audio.astype(np.float32) - 128.0) / 128.0
                            elif full_audio.dtype == np.int16:
                            ...
                            # Calculate start and end samples based on motion timing
                            start_sample = int(start_time * sr)
                            end_sample = start_sample + int(video_duration * sr)
                            ...
                            if end_sample <= len(full_audio):
                                ...
                                waveform = full_audio[start_sample:]
                                pad_len = end_sample - start_sample - len(waveform)
                                waveform = np.concatenate([waveform, np.zeros(pad_len)])
                        else:
                            # Fall back to using the waveform from audio_meta
                            if "waveform" not in audio_meta:
                                raise ValueError("No waveform or valid audio path available")
                            waveform = audio_meta["waveform"]
                            if hasattr(waveform, "cpu"):
                                waveform = waveform.cpu().numpy()
                            sr = int(audio_meta["sample_rate"])

                            # Pre-check dtype if possible, or just force float
                            # Convert to float32
                            ...
                            if len(waveform) > expected_samples:
                                ...
                                pad_len = expected_samples - len(waveform)
                                waveform = np.concatenate([waveform, np.zeros(pad_len)])
                        ...
                        print(f"[DEBUG Audio] Sample {sample_i} Rep {rep_i}: min={waveform.min():.4f}, max={waveform.max():.4f}, mean={waveform.mean():.4f}, std={waveform.std():.4f}, dtype={waveform.dtype}")
                        
                        # DEBUG: Save intermediate wav file
                        debug_wav_path = os.path.join(out_path, f"debug_audio_sample_{sample_i}_rep_{rep_i}.wav")
                        from scipy.io import wavfile
                        # Save as float32 wav
                        wavfile.write(debug_wav_path, sr, waveform)
                        print(f"[DEBUG Audio] Saved debug audio to {debug_wav_path}")

                        # Create audio clip
                        audio_clip = AudioArrayClip(waveform, fps=sr)
                        
                        # Attach audio to video clip
                        video_clip = animations[sample_i, rep_i]
                        animations[sample_i, rep_i] = video_clip.set_audio(audio_clip)
                    except Exception as e:
                        print(
                            f"[Warning] Failed to add audio to sample {sample_i} rep {rep_i}: {e}"
                        )
...
def save_multiple_samples(
    ...
    n_samples, n_reps = animations.shape
    
    # Calculate correct duration
    expected_duration = max_frames / fps
    print(f"[Info] Saving {n_samples * n_reps} individual video files with duration {expected_duration:.2f}s")

    for sample_i in range(n_samples):
        for rep_i in range(n_reps):
            clip = animations[sample_i, rep_i]
            if clip is None:
                continue
            
            # Construct filename: sampleXX_repYY.mp4
            save_file = f"sample{sample_i:02d}_rep{rep_i:02d}.mp4"
            save_path = os.path.join(out_path, save_file)
            
            print(f"Saving individual clip: {save_file}")

            # Write temp file for individual clip (VIDEO ONLY)
            clip = clip.set_audio(None) # Remove audio from moviepy rendering to be safe
            temp_path = save_path.replace('.mp4', '_temp.mp4')
            try:
                clip.write_videofile(
                    temp_path, fps=fps, audio_codec=None, threads=4, logger=None
                )
            except Exception as e:
                print(f"[Error] Failed to write temp file for {save_file}: {e}")
                continue

            # Cleanup clip memory
            clip.close()

            # Identify corresponding debug audio file
            # Filename format from main loop: debug_audio_sample_{sample_i}_rep_{rep_i}.wav
            debug_audio_path = os.path.join(out_path, f"debug_audio_sample_{sample_i}_rep_{rep_i}.wav")
            
            # Post-process with FFmpeg: Mux video from temp_path and audio from debug_audio_path
            import subprocess
            
            if os.path.exists(debug_audio_path):
                print(f"[Info] Muxing clean audio from {os.path.basename(debug_audio_path)}")
                ffmpeg_cmd = [
                    'ffmpeg', '-y',
                    '-i', temp_path,           # Input 0: Video
                    '-i', debug_audio_path,    # Input 1: Audio (Clean WAV)
                    '-t', str(expected_duration), # Trim total duration
                    '-map', '0:v:0',           # Use video from Input 0
                    '-map', '1:a:0',           # Use audio from Input 1
                    '-c:v', 'copy',            # Copy video stream
                    '-c:a', 'aac',             # Encode audio to AAC (high quality default)
                    '-b:a', '192k',            # Force high bitrate
                    save_path
                ]
            else:
                print(f"[Warning] Debug audio not found at {debug_audio_path}, saving video without audio.")
                ffmpeg_cmd = [
            ...
            try:
                ...
                print(f"[Error] FFmpeg failed for {save_file}. Stderr: {e.stderr.decode()}")
...
def load_dataset(args, max_frames, n_frames):
    data = get_dataset_loader(
        ...
        pred_len=max_frames,
        fixed_len=max_frames + args.context_len,  # TODO
        device=dist_util.dev(),
    )
