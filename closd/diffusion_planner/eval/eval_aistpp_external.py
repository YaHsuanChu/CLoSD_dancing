...
def _to_263_tn(motion: torch.Tensor) -> torch.Tensor:
    ...
    raise ValueError(
        f"Unsupported motion dim: {motion.dim()} shape={tuple(motion.shape)}"
    )
...
def _recover_joints_from_263(
    ...
    joints = recover_from_ric(
        motion_tn_263, n_joints, hml_type
    )  # (B, T, n_joints, 3)
...
def _load_external_results_npy(
    path: str, use_gt: bool = False
) -> Tuple[np.ndarray, np.ndarray, Optional[List[Dict]]]:
    ...
    if audio_waveforms is not None:
        print(
            f"[INFO] Loaded {len(audio_waveforms)} paired audio waveforms from npy file"
        )
...
def _extract_beats_from_waveform(
    ...
    try:
        ...
        idx = (
            np.where(
                (env[1:-1] > env[:-2]) & (env[1:-1] > env[2:]) & (env[1:-1] > thr)
            )[0]
            + 1
        )
...
def _extract_kinematic_beats(
    joints: np.ndarray, fps: float = 60.0, smooth: bool = False
) -> np.ndarray:
...
def _beat_align_score(
    music_beats: np.ndarray,
    dance_beats: np.ndarray,
    sigma: float = 0.05,
) -> Dict[str, float]:
    """Calculate Beat Alignment Score (BAS).

    Calculates:
    - "precision" (AIST++ 2021, FACT): Dance -> Music. Penalizes extra dance beats.
    - "recall" (Bailando 2022, EDGE 2023 SOTA): Music -> Dance.
      Checks if each music beat has a corresponding dance beat.
    - "f1": Harmonic mean of Precision and Recall.

    Args:
        music_beats: Times of music beats (seconds)
        dance_beats: Times of kinematic beats / velocity minima (seconds)
        sigma: Gaussian tolerance in seconds (AIST++ standard: 3 frames ~ 0.05s at 60fps)

    Returns:
        Dict with keys "precision", "recall", "f1". Scores in [0, 1].
    """
    if dance_beats.size == 0 or music_beats.size == 0:
        return {"precision": 0.0, "recall": 0.0, "f1": 0.0}

    # Matrix: [Num_Music_Beats, Num_Dance_Beats]
    diff = np.abs(music_beats[:, None] - dance_beats[None, :])

    # Precision: For each DANCE beat, find nearest MUSIC beat
    # Penalizes "off-beat" movements
    min_dist_prec = diff.min(axis=0)  # min across music beats for each dance beat
    prec = np.exp(-(min_dist_prec**2) / (2 * sigma**2)).mean()

    # Recall: For each MUSIC beat, find nearest DANCE beat
    # Checks if dancer "hits" each beat
    min_dist_rec = diff.min(axis=1)  # min across dance beats for each music beat
    rec = np.exp(-(min_dist_rec**2) / (2 * sigma**2)).mean()
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to results.npy from sample/generate.py",
    )
    ...
    parser.add_argument(
        "--fps", type=float, default=60.0, help="Motion FPS (AIST++ uses 60fps)"
    )
    parser.add_argument("--diversity_times", type=int, default=300)
    parser.add_argument("--beat_sigma", type=float, default=0.05)
    parser.add_argument(
        "--smooth_kinematic",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of samples for BeatAlign calculation (default: -1 for all)",
    )
    parser.add_argument(
        "--beat_mode",
        type=str,
        default="recall",
        help="DEPRECATED: Script now calculates all beat metrics (Precision/Recall/F1).",
    )
    parser.add_argument(
        "--verbose_beat",
        action="store_true",
        help="Print per-sample BeatAlign scores for debugging",
    )
    ...
    # external motions
    motions_np, lengths_np, paired_audio = _load_external_results(
        args.external_results_file, use_gt=args.eval_gt
    )
    ...
    if lengths.max() > actual_motion_len:
        print(
            f"[WARN] lengths max ({lengths.max().item()}) > actual_motion_len ({actual_motion_len}), clamping"
        )
    ...
    if paired_audio is not None:
        ...
        print(
            f"[INFO] Using {len(paired_audio)} paired audio waveforms for beat alignment"
        )
        ...
        if not args.skip_fid:
            audio_fixed_len = (
                actual_motion_len
                if actual_motion_len > args.fixed_len
                else args.fixed_len
            )
        ...
        print(
            f"[INFO] Loading {audio_samples} audio samples for BeatAlign (fixed_len={audio_fixed_len})"
        )
    ...
    if _is_joints_motion(motions):
        ...
        diversity = (
            float(calculate_diversity(gen_emb, div_times)) if div_times > 1 else 0.0
        )
    ...
    if os.path.exists(mean_path) and os.path.exists(std_path):
        ...
        print(
            f"[WARN] Could not find mean/std files at {dataset_dir}, joints may be incorrect scale"
        )
    ...
    beat_loop_limit = (
        args.beat_samples
        if args.beat_samples > 0
        else min(num_samples, len(gt_audio_meta))
    )
    # Ensure we don't go out of bounds of available data
    beat_loop_limit = min(beat_loop_limit, num_samples, len(gt_audio_meta))

    for i in range(beat_loop_limit):
        ...
        kinematic_beats = _extract_kinematic_beats(
            joints_i, fps=args.fps, smooth=args.smooth_kinematic
        )

        # Calculate alignment: how well do kinematic beats align with music beats?
        scores = _beat_align_score(music_beats, kinematic_beats, sigma=args.beat_sigma)

        scores_precision.append(scores["precision"])
        scores_recall.append(scores["recall"])
        scores_f1.append(scores["f1"])

        if args.verbose_beat:
            print(
                f"[{i:4d}] music_beats={len(music_beats):3d} kinematic_beats={len(kinematic_beats):3d} | "
                f"P={scores['precision']:.4f} R={scores['recall']:.4f} F1={scores['f1']:.4f}"
            )

    if n_librosa_fallback > 0:
        print(
            f"[WARN] librosa not used for {n_librosa_fallback}/{n_with_audio} samples (fallback peak detection used)"
        )
    ...
    mean_p, std_p = get_stats(scores_precision)
    mean_r, std_r = get_stats(scores_recall)
    mean_f1, std_f1 = get_stats(scores_f1)
    ...
    print(f"--- Beat Alignment (sigma={args.beat_sigma}, n={len(scores_f1)}) ---")
